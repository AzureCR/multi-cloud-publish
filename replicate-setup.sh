...
FunctionAppStorageName=replicatefuncstore
FunctionAppName=replicateFunction
ConfigKeyVaultName=funcAppConfigVault
...
FunctionAppObjectId=$(az functionapp create -n $FunctionAppName --storage-account $FunctionAppStorageName --consumption-plan-location $SourceRegion --runtime dotnet -g $SourceResourceGroupName --functions-version 3 --assign-identity [system] --query identity.principalId -o tsv)
...
FunctionAppResourceId=$(az functionapp show --name $FunctionAppName -g $SourceResourceGroupName --query id --output tsv)
